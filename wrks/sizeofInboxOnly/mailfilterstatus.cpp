  ...
  ui(new Ui::MailFilterStatus)
{
  ...
  system("~smagri/proj/mfilterStatus/c++Qt/MailFilterStatus/myScript");
  fprintf(stderr, "after system\n");
  sleep(3);
  

  if ( (fileSPtr =
	fopen("/lu1/smagri/proj/mfilterStatus/c++Qt/MailFilterStatus/MFSsizes.txt",
	      "r")) == NULL ){
    perror("MailFilterStatus");
    fprintf(stderr, "MailFilterStatus: Error Opening MFSsizes.txt\n");
    exit(1);
  }
  
  fprintf(stderr, "after fopen\n");
  retval = fscanf(fileSPtr, "%d", &SOInbox);
  fprintf(stderr, "after fscanf\n");
  fprintf(stderr, "retval=%d\n", retval);
  // sizeofInbox = 0;
  ui->numEmailsInInbox->setText(QString::number(SOInbox));
