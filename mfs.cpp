  ...
  ui(new Ui::MFS)
{
  ...
  lastDeltaAccessTime[2] = fStats[2].st_mtime;
  ...
  lastDeltaAccessTime[5] = fStats[5].st_mtime;
  ...
  strcpy (plain_stray, RUN_DIR);
  strcat(plain_stray, "getMFSsizes");
  if (system(plain_stray) == -1){
    fprintf(stderr, "MFS: Error executing getMFSsizes\n");
    exit(2);
  }
  
  // Note, the file path must be an absolute path, so using ~smagri !wrks.
  strcpy (plain_stray, RUN_DIR);
  strcat(plain_stray, "MFSsizes.txt");
  if ( (fileSPtr = fopen(plain_stray, "r")) == NULL ){
    perror("MFS");
...
void MFS::timer_cbk()
{
  ...
  // Colour/rgb numbers obtained via gimp.
  ...
  for (i=0; i<NUM_SPOOL_FILES; i++){
	...
	lastDeltaAccessTime[i] = fStats[i].st_mtime;
	lastDeltaBGcolour[i] = 0;
      }
	...
	if (lastDeltaBGcolour[i] == 0){
	  // No  distinction is  made wrt  last delta  in  BGcolour when
	  // spool file has  been accessed yet not reduced  in size.  In
	  // this case we just make the BGcolour another shade of pink.
	  //
	  // Changes in  access time  on a spool  file might  occur much
	  // later than the actual access  time, i have observed about a
	  // 1min lag.
	  //
	  // Time arguments  in difftime()  should never overflow  as t1
	  // and    t2   are    #seconds   since    the   epoch/calender
	  // time/UTC/Coordinated  Universal  Time(transends timezones).
	  // Well it will but not in my lifetime. 
	  if (difftime(fStats[i].st_mtime, lastDeltaAccessTime[i]) > 0){
	    labelPtrs[i]->
	      setStyleSheet("QLabel {background-color: rgb(255, 180, 174);}");
	    // dbg: testing colour
	    //labelPtrs[i]->
	    //  setStyleSheet("QLabel {background-color: rgb(255, 10, 10);}");
	  }
	}
      }
    }
...
void MFS::on_numEmailsInInbox_linkActivated(const QString &link)
{
